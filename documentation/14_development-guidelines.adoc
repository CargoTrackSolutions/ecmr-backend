...
This chapter serves as an onboarding guide for new developers and outlines the prerequisites, development setup, and key concepts required to start contributing to the eCMR project.

=== Docker Compose Setup (Development Only)

We use `docker-compose.yml` for local development and testing. This setup includes application containers and essential services like databases or message queues.
...
To add other users you must add them to realm config file and to the backend project (`src/main/resources/db/changelog/02-init-data.xml`). Please verify that the email addresses are identical.
...
Docker must be available to run the integration tests (src/test/e2e) as it is used to start the Keycloak test container.
