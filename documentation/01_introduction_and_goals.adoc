...
This section describes the background and origin of the eCMR.

=== Project description

The eCMR (electronic consignment note) is an open source standard and software solution developed by the Open Logistics Foundation to digitally modernize the international consignment note (CMR) and enable seamless, interoperable transport documentation across the supply chain.

*Who built it?*
More than 28 logistics service providers, transport platforms, and logistics and IT service providers worked together on the new standard.
These organizations represent a wide range of European countries and contribute diverse perspectives based on company size, operational roles, and real-world practice.

*Why was it built?*
This project was initiated to address the ongoing challenges of paper-based transport documentation and fragmented digital solutions.
The goal was to create a shared, open, and interoperable approach without forcing organizations into proprietary systems or vendor lock-ins.

*For whom is it intended?*
The eCMR is designed for all key stakeholders involved in road freight transport, including:

- logistics service providers and carriers
- shippers and consignees
- transport platforms and freight networks
- IT and software providers supporting transport processes
...
Today, CMRs are usually filled out manually and analog on paper, which creates a large effort when trying to handle all documents included in one shipment.
In practice, there is also not a consistent layout of the CMR, neither in national nor in international transportation.
Furthermore, the CMR is often not the only document carried along with a shipment — usually further supporting documents such as documents relating to dangerous goods or samples are included.
...
The CMR note is legally recognized in over 55 countries around the world.

=== eCMR as a Solution to Prevalent Problems
...
The administration expense is thereby reduced by between 4 € and 13 €, which results in a cost reduction of 50 % compared to the paper-based version.
Furthermore, the improvement in efficiency and sustainability resulting from the digitalization of cargo transportation and logistics services leads to time saved in information transfer and reduced use in resources since paper is no longer necessary.
Another goal of the eCMR is to eliminate unauthorized third-party access.
Thus, an access security system (authentication service) with specifically defined access rights for authorized parties is implemented.
The risk of media breaks, transmission errors, and misinformation is also reduced because subsequent editing of the eCMR is possible, while previously saved information remains uncompromisable.
Additional features such as digital stamping are available.
All processes along the route of transportation are saved in the processing protocol along with the data set.

=== Electronic Seals as digital signatures

This project uses electronic seals (eSeals) to create digital signatures in compliance with the EU eIDAS Regulation
(Regulation (EU) No 910/2014). Further information about this concept is available here: <<eseal>>.
...
.. In Germany the German Bundestag has ratified on 27.09.2021 the Additional Protocol to the Convention on the
Contract for the International Carriage of Goods by Road (CMR) concerning the electronic consignment note
(http://www.bgbl.de/xaver/bgbl/start.xav?startbk=Bundesanzeiger_BGBl&jumpTo=bgbl221s1035.pdf[source]).

=== Quality Goals

The following table describes the top six quality goals of eCMR classified by ISO/IEC 9126-1 characteristics.
...
a| CMR template of International Road Transport Union (IRU) that has been available since 2007 and is widely prevalent
in paper based CMRs
